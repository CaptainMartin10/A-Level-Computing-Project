...
namespace A_Level_Computing_Project
{
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (mouseState.LeftButton == ButtonState.Pressed)
            {
                foreach (Province Hex in MapArray)
                {
                    if (Hex.X % 2 == 0)
                    {
                        var HexBounds1 = new Rectangle((Hex.X * 27) + 9, (Hex.Y * 36), 18, 36);
                        var HexBounds2 = new Rectangle((Hex.X * 27) + 8, (Hex.Y * 36) + 1, 20, 34);
                        var HexBounds3 = new Rectangle((Hex.X * 27) + 7, (Hex.Y * 36) + 3, 22, 30);
                        var HexBounds4 = new Rectangle((Hex.X * 27) + 6, (Hex.Y * 36) + 5, 24, 26);
                        var HexBounds5 = new Rectangle((Hex.X * 27) + 5, (Hex.Y * 36) + 7, 26, 22);
                        var HexBounds6 = new Rectangle((Hex.X * 27) + 4, (Hex.Y * 36) + 9, 28, 18);
                        var HexBounds7 = new Rectangle((Hex.X * 27) + 3, (Hex.Y * 36) + 1, 30, 14);
                        var HexBounds8 = new Rectangle((Hex.X * 27) + 2, (Hex.Y * 36) + 3, 32, 10);
                        var HexBounds9 = new Rectangle((Hex.X * 27) + 1, (Hex.Y * 36) + 5, 34, 6);
                        var HexBounds10 = new Rectangle((Hex.X * 27), (Hex.Y * 36) + 7, 36, 2);
                    ...
                    {
                        var HexBounds1 = new Rectangle((Hex.X * 27) + 9, (Hex.Y * 36) + 18, 18, 36);
                        var HexBounds2 = new Rectangle((Hex.X * 27) + 8, (Hex.Y * 36) + 19, 20, 34);
                        var HexBounds3 = new Rectangle((Hex.X * 27) + 7, (Hex.Y * 36) + 21, 22, 30);
                        var HexBounds4 = new Rectangle((Hex.X * 27) + 6, (Hex.Y * 36) + 23, 24, 26);
                        var HexBounds5 = new Rectangle((Hex.X * 27) + 5, (Hex.Y * 36) + 25, 26, 22);
                        var HexBounds6 = new Rectangle((Hex.X * 27) + 4, (Hex.Y * 36) + 27, 28, 18);
                        var HexBounds7 = new Rectangle((Hex.X * 27) + 3, (Hex.Y * 36) + 29, 30, 14);
                        var HexBounds8 = new Rectangle((Hex.X * 27) + 2, (Hex.Y * 36) + 31, 32, 10);
                        var HexBounds9 = new Rectangle((Hex.X * 27) + 1, (Hex.Y * 36) + 33, 34, 6);
                        var HexBounds10 = new Rectangle((Hex.X * 27), (Hex.Y * 36) + 35, 36, 2);
                        if (HexBounds1.Contains(mousePoint) || HexBounds2.Contains(mousePoint) || HexBounds3.Contains(mousePoint) || HexBounds4.Contains(mousePoint) || HexBounds5.Contains(mousePoint) || HexBounds6.Contains(mousePoint) || HexBounds7.Contains(mousePoint) || HexBounds8.Contains(mousePoint) || HexBounds9.Contains(mousePoint) || HexBounds10.Contains(mousePoint))
                        {
                            CurrentHexX = Hex.X;
                            CurrentHexY = Hex.Y;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            _spriteBatch.DrawString(MenuFont, "Next Turn", new Vector2(666, 623 ), Color.White);
            _spriteBatch.End();
